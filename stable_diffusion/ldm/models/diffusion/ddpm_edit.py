...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.GuidanceNet = DiffusionWrapper(add_unet_config,conditioning_key)
...
class LatentDiffusion(DDPM):
    ...
    def p_mean_variance(self, x, c, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
                        return_x0=False, score_corrector=None, corrector_kwargs=None,is_ref = False):
        t_in = t
        model_out = self.apply_model(x, t_in, c, return_ids=return_codebook_ids)
        #:( 在这里加入之后，就会在之后的单步之中被用于预测下一步的
    ...
    def configure_optimizers(self):
        lr = self.learning_rate
        params = list(self.GuidanceNet.parameters())
        #直接将源代码中的model改成新网络就会只更新我们的网络了么
        if self.cond_stage_trainable:
